...
app.get("/products/:id", (req, res) => {
  const query = `
        SELECT p.product_id, p.product_name, p.product_description, p.product_price, p.stock,
...
app.get("/cart", authenticate, (req, res) => {
  ...
  db.query(query, [userId], (err, results) => {
    ...
    // Always return an array; let frontend handle empty state
    res.json(results);
  });
});
app.get("/cart/count", authenticate, (req, res) => {
  ...
  console.log("[/cart/count] Called for userId:", userId);
  ...
  db.query(query, [userId], (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to retrieve cart count", details: err });
    }

    console.log("[/cart/count] Raw DB results:", results);
    const count = (results && results[0] && results[0].count) || 0;
    console.log("[/cart/count] Final count for userId", userId, "=", count);
    ...
    if (results.length > 0) {
      ...
      db.query(createCartQuery, [userId], (err, result) => {
        if (err)
          return res
            .status(500)
            .json({ error: "Failed to create cart", details: err });
  ...
  const addCartItem = (cartId) => {
    ...
    db.query(addItemQuery, [cartId, product_id, quantity], (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Failed to add item to cart", details: err });
...
app.get("/wishlist", authenticate, (req, res) => {
  ...
  db.query(query, [userId], (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Failed to retrieve wishlist items", details: err });
...
app.get("/wishlist/count", authenticate, (req, res) => {
  const userId = req.userId;
  ...
  db.query(query, [userId], (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Failed to retrieve wishlist count", details: err });
...
app.post("/wishlist", authenticate, (req, res) => {
  ...
  db.query(query, [userId, product_id], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Failed to add item to wishlist", details: err });
...
app.delete("/wishlist/:wishlistId", authenticate, (req, res) => {
  ...
  db.query(query, [wishlistId], (err, result) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Failed to remove item from wishlist", details: err });
...
app.post("/login", (req, res) => {
  ...
  db.query(query, [email], async (err, results) => {
    ...
    // Generate JWT token
    const token = jwt.sign({ userId: user.customer_id }, SECRET_KEY, {
      expiresIn: "5h",
    });
...
app.get("/cart/total", authenticate, (req, res) => {
  ...
  db.query(query, [userId], (err, results) => {
    if (err)
      return res
        .status(500)
        .json({ error: "Failed to calculate cart total", details: err });
    console.log(results);
...
app.post("/checkout", authenticate, (req, res) => {
  ...
  if (!payment_method || !address || !city || !postal_code || !phone_number) {
    return res
      .status(400)
      .json({ error: "All payment and shipment details are required" });
  }
      ...
      db.query(
        ...
        (err) => {
          if (err)
            return res
              .status(500)
              .json({ error: "Failed to add payment", details: err });
          ...
          db.query(
            ...
            (err) => {
              if (err)
                return res
                  .status(500)
                  .json({ error: "Failed to add shipment", details: err });
              ...
              db.query(clearCartQuery, [cartId], (err) => {
                if (err)
                  return res
                    .status(500)
                    .json({ error: "Failed to clear cart", details: err });
